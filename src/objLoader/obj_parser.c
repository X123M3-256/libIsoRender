/*Obj loader code from http://kixor.net/dev/objloader/*/
...
static obj_face* obj_parse_face(obj_growable_scene_data* scene)
{
    ...
    vertex_count = obj_parse_vertex_index(face->vertex_index, face->texture_index,
        face->normal_index);
    obj_convert_to_list_index_v(scene->vertex_list.item_count,
        face->vertex_index);
    obj_convert_to_list_index_v(scene->vertex_texture_list.item_count,
        face->texture_index);
    obj_convert_to_list_index_v(scene->vertex_normal_list.item_count,
        face->normal_index);
...
static obj_sphere* obj_parse_sphere(obj_growable_scene_data* scene)
{
    ...
    obj->up_normal_index = obj_convert_to_list_index(
        scene->vertex_normal_list.item_count, temp_indices[1]);
    obj->equator_normal_index = obj_convert_to_list_index(
        scene->vertex_normal_list.item_count, temp_indices[2]);
...
static obj_plane* obj_parse_plane(obj_growable_scene_data* scene)
{
    ...
    obj_convert_to_list_index_v(scene->vertex_texture_list.item_count,
        obj->texture_index);
    obj->pos_index = obj_convert_to_list_index(scene->vertex_list.item_count, temp_indices[0]);
    obj->normal_index = obj_convert_to_list_index(
        scene->vertex_normal_list.item_count, temp_indices[1]);
    obj->rotation_normal_index = obj_convert_to_list_index(
        scene->vertex_normal_list.item_count, temp_indices[2]);
...
static obj_vector* obj_parse_vector()
{
    ...
    char* z=strtok(NULL, WHITESPACE);
    v->e[2] = z==NULL?0.0:atof(z);
    return v;
}
...
static void obj_parse_camera(obj_growable_scene_data* scene, obj_camera* camera)
{
    ...
    camera->camera_up_norm_index = obj_convert_to_list_index(
...
static int obj_parse_mtl_file(char* filename, list* material_list)
{
    ...
    while (fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream)) {
        ...
        // texture map
            ...
            fprintf(stderr,
                "Unknown command '%s' in material file %s at line %i:\n\t%s\n",
...
static int obj_parse_obj_file(obj_growable_scene_data* growable_data, const char* filename)
{
    ...
    /*
          extreme_dimensions[0].x = INFINITY; extreme_dimensions[0].y =
     INFINITY; extreme_dimensions[0].z = INFINITY;
          extreme_dimensions[1].x = -INFINITY; extreme_dimensions[1].y =
     -INFINITY; extreme_dimensions[1].z = -INFINITY;

                          if(v->x < extreme_dimensions[0].x)
     extreme_dimensions[0].x = v->x;
                          if(v->x > extreme_dimensions[1].x)
     extreme_dimensions[1].x = v->x;
                          if(v->y < extreme_dimensions[0].y)
     extreme_dimensions[0].y = v->y;
                          if(v->y > extreme_dimensions[1].y)
     extreme_dimensions[1].y = v->y;
                          if(v->z < extreme_dimensions[0].z)
     extreme_dimensions[0].z = v->z;
                          if(v->z > extreme_dimensions[1].z)
     extreme_dimensions[1].z = v->z;*/
    ...
    while (fgets(current_line, OBJ_LINE_SIZE, obj_file_stream)) {
        ...
        {
            list_add_item(&growable_data->vertex_normal_list, obj_parse_vector(),
                NULL);
        }

        else if (strequal(current_token, "vt")) // process vertex texture
        {
            list_add_item(&growable_data->vertex_texture_list, obj_parse_vector(),
                NULL);
        }
        ...
        {
            strncpy(growable_data->material_filename, strtok(NULL, WHITESPACE),
                OBJ_FILENAME_LENGTH);
            obj_parse_mtl_file(growable_data->material_filename,
                &growable_data->material_list);
            continue;
        }
...
int parse_obj_scene(obj_scene_data* data_out, const char* filename)
{
    ...
    // print_vector(NORMAL, "Max bounds are: ",
    // &growable_data->extreme_dimensions[1]);
    // print_vector(NORMAL, "Min bounds are: ",
    // &growable_data->extreme_dimensions[0]);
