...
int texture_load_png(texture_t* texture, const char* filename)
{
	...
	if (setjmp(png_jmpbuf(png)))
		abort(); // TODO Not sure what this does but I don't think it's what I want
	...
	for (int y = 0; y < texture->height; y++)
		...
		{
			color_t color = { (uint8_t)row_pointers[y][4 * x], (uint8_t)row_pointers[y][4 * x + 1],
					  (uint8_t)row_pointers[y][4 * x + 2] };
...
int mesh_load(mesh_t* output, const char* filename)
{
	const struct aiScene* scene = aiImportFile(
	    filename, aiProcess_Triangulate | aiProcess_JoinIdenticalVertices | aiProcess_GenNormals);
	...
	for (uint32_t i = 0; i < scene->mNumMaterials; i++)
	{
		...
		if (aiGetMaterialString(mat, AI_MATKEY_NAME, &name) == AI_SUCCESS)
		{
			...
			else if (strstr(name.data, "Chain1") != NULL)
				output->materials[i].region = 5;
			else if (strstr(name.data, "Chain2") != NULL)
				output->materials[i].region = 6;
			else if (strstr(name.data, "Chain3") != NULL)
				output->materials[i].region = 7;
			if (strstr(name.data, "Mask") != NULL)
				output->materials[i].flags |= MATERIAL_IS_MASK;
			if (strstr(name.data, "NoAO") != NULL)
				output->materials[i].flags |= MATERIAL_NO_AO;
			if (strstr(name.data, "BackgroundAA") != NULL)
				output->materials[i].flags |= MATERIAL_BACKGROUND_AA;
			// printf("%s\n",name.data);
		}
		...
		if (aiGetMaterialColor(mat, AI_MATKEY_COLOR_SPECULAR, &specular) == AI_SUCCESS)
		{
			...
			{
				output->materials[i].specular_color = vector3_mult(
				    output->materials[i].specular_color, specular_strength);
			}
	...
	for (uint32_t j = 0; j < scene->mNumMeshes; j++)
	{
		...
		{
			output->vertices[mesh_start_vertex + i] = vector3(
			    mesh->mVertices[i].x, mesh->mVertices[i].y, mesh->mVertices[i].z);
			output->normals[mesh_start_vertex + i] = vector3(
			    mesh->mNormals[i].x, mesh->mNormals[i].y, mesh->mNormals[i].z);
			if (mesh->mTextureCoords[0])
			{
				output->uvs[mesh_start_vertex + i] = vector2(
				    mesh->mTextureCoords[0][i].x, mesh->mTextureCoords[0][i].y);
			}
			else
				output->uvs[mesh_start_vertex + i] = vector2(0.0, 0.0);
		}
		...
		for (uint32_t i = 0; i < mesh->mNumFaces; i++)
		{
			...
			for (uint32_t j = 0; j < 3; j++)
				output->faces[mesh_start_face + i].indices[j] = mesh_start_vertex + mesh->mFaces[i].mIndices[j];
		}
